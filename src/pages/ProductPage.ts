...
export class ProductsPage {
    ...
    constructor(page: Page) {
        ...
        const firstCardVisibleContainer = this.productCards.nth(0).locator('.productinfo');
        ...
        this.viewProductButton = this.productCards.nth(0).getByRole('link', { name: 'View Product' });
        ...
        this.detailAvailability = detailBlock.locator('p:has-text("Availability:")');
        ...
        // Search view targets
        this.searchedProductsHeader = this.productSection.getByRole('heading', { name: 'Searched Products', exact: false });
        ...
        this.firstCardHoverContainer = this.productCards.nth(0).locator('.product-overlay');
        // Target the overlay's specific add-to-cart anchor button link
        this.addToCartOverlayButton = this.firstCardHoverContainer.locator('a.add-to-cart');
        ...
        this.continueShoppingButton = this.successModal.getByRole('button', { name: 'Continue Shopping' });
    ...
    async verifyAllProductsHeaderVisible(): Promise<void> {
        const allProductsHeader = this.productSection.getByRole('heading', { name: 'All Products', exact: false });
    ...
    async verifyDetailAttributes(expectedName: string, expectedPrice: string): Promise<void> {
    ...
    async verifyProductMetadataLayout(): Promise<void> {
        ...
        await expect(this.detailAvailability).toHaveText(/^Availability:\s*[a-zA-Z ]+$/);
    ...
    async verifySearchResultsKeyword(keyword: string): Promise<void> {
        ...
        const firstProductLink = this.productCards.nth(0).getByRole('link', { name: 'View Product' });
        ...
        const categoryText = await detailBlock.locator('p:has-text("Category:")').innerText();
        ...
        const isKeywordMatched = titleText.toLowerCase().includes(keyword.toLowerCase()) || 
    ...
    async selectBrandFilter(brandName: string): Promise<void> {
        ...
        const brandLink = this.brandFilterBlock.getByRole('link', { name: brandName, exact: false });
    ...
    async verifyAllCardsBelongToBrand(brandName: string): Promise<void> {
        ...
        if (count > 0) {
            ...
            await this.productCards.nth(0).getByRole('link', { name: 'View Product' }).click();
            await expect(this.page).toHaveURL(/.*product_details.*/);
